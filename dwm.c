...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		while (1) {
			...
			ctmp = *ts;
			*ts = '\0';
			drw_text(drw, m->ww - sw - stw + tx, 0, sw - tx, bh, lrpad / 2 - 2, tp, 0);
			tx += TEXTW(tp) -lrpad;
			if (ctmp == '\0') { break; }
...
void
runAutostart(void) {
    /* Executable present in $PATH for startup applications. */
	int err = system("autostart.sh");
	if (err != 0 )
	    perror("system");
}
